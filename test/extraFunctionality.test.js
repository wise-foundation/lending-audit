...
contract("extra Functionality", async accounts  => {
    ...
    describe("payback on behalf with lendingshares Tests", () => {
        ...
        it("payback with lendingshares same user", async () => {
        ...
        it("payback with lendingshares different user", async () => {
        ...
        it("payback borrowShares correctly cancelled and checked for borrowlimit", async () => {
            ...
            await expectRevert(
                contracts.lending.paybackExactLendingShares(
                    2,
                    1,
                    WETH.address,
                    lendingShareToTransform,
            ...
            const borrowShareAfterUser = await contracts.lending.getPositionBorrowShares(
                1,
                WETH.address
            );

            const borrowSharesCancelled = Bi(borrowShareBeforeUser)
                - Bi(borrowShareAfterUser);

            const diff = abs(
                Bi(borrowSharesCancelled) - Bi(borrowSharesToCancelCalculated)
            );
        ...
        it("payback with lendingshares lifecycle test", async () => {
    ...
    describe("withdrawOnBehalf Tests", () => {
        ...
        it("Allowance for withdraw gets reduced correctly", async () => {
            ...
            const valueWithdraw = await contracts.lending.withdrawOnBehalfExactShares.call(
                1,
                WETH.address,
                percentageShare,
                {
            ...
            await contracts.lending.withdrawOnBehalfExactShares(
                1,
                WETH.address,
                percentageShare,
                {
        ...
        it("User cant withdraw more with withdrawOnBehalf than dept ratio allows", async () => {
            ...
            await expectRevert(
                contracts.lending.withdrawOnBehalfExactShares(
                    1,
                    WETH.address,
                    withdrawShares,
                    {
        ...
        it("Lending shares get reduced correctly with withdrawOnBehalf (or amount)", async () => {
            ...
            await contracts.lending.withdrawOnBehalfExactAmount(
                1,
                WETH.address,
                withdrawAmount,
                {
            ...
            let lendingEntry = await contracts.lending.userLendingData(
                1,
                WETH.address
            );
            ...
            const userShares = await contracts.lending.getPositionLendingShares(
                1,
                WETH.address
            );
    ...
    describe("BorrowOnBehalf tests", () => {
        ...
        it("Borrow shares get calculated correctly with borrowOnBehalfExactAmount", async () => {
            ...
            let share1 = await contracts.lending.getPositionBorrowShares(
                1,
                WETH.address
            );
            ...
            const totalShare = await contracts.lending.getTotalBorrowShares(
                WETH.address
            );

            const totalBorroPseudo = await contracts.lending.getPseudoTotalBorrowAmount(
                WETH.address
            );

            const calcValue = Bi(borrowAmount)
                * Bi(totalShare)
                / Bi(totalBorroPseudo);
            ...
            share2 = await contracts.lending.getPositionBorrowShares(
                1,
                WETH.address
            );
        ...
        it.skip("Borrow amount get calculated correctly with borrowOnBehalfExactShares", async () => {
            ...
            await contracts.lending.borrowOnBehalfExactShares(
                1,
                WETH.address,
                sharesToBorrow,
                {
