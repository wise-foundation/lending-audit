...
const OracleHub = artifacts.require("WiseOracleHub");
...
contract("extra Functionality2", async accounts  => {
    ...
    describe("Collateral for solely borrow tests", () => {
        ...
        it("Total bare token gets increased correctly", async () => {
            ...
            const pureCollat1 = await contracts.lending.positionPureCollateralAmount(
                1,
                WETH.address
            );

            const pureCollat2 = await contracts.lending.positionPureCollateralAmount(
                1,
                USDC.address
            );
        ...
        it("Collateral gets calculated correctly (only solely borrow deposit)", async () => {
            ...
            const result = await contracts.security.overallUSDCollateralsBoth(
                1
            );

            const {0: weighted, 1: bare} = result;

            const weightedOtherFunc = await contracts.security.overallUSDCollateralsWeighted.call(
                1
            );
            ...
            * Bi(10 ** (18 - decUSDC))
            ...
            const result2 = await contracts.security.overallUSDCollateralsBoth(
                1
            );

            const {0: weighted2, 1: bare2} = result2;

            const weightedOtherFunc2 = await contracts.security.overallUSDCollateralsWeighted.call(
                1
            );
        ...
        it("Collateral gets calculated correctly (normal plus solely borrow deposit)", async () => {
            ...
            const result1 = await contracts.security.overallUSDCollateralsBoth(
                1
            );

            const {0: weighted1, 1: bare1} = result1;

            const weightedOtherFunc1 = await contracts.security.overallUSDCollateralsWeighted.call(
                1
            );
            ...
            const resultEnd = await contracts.security.overallUSDCollateralsBoth(
                1
            );

            const {0: weightedEnd, 1: bareEnd} = resultEnd;

            const weightedOtherFuncEnd = await contracts.security.overallUSDCollateralsWeighted.call(
                1
            );

            assert.equal(
                weightedEnd.toString(),
                calcWeightedDepositEndSum.toString()
            );
        ...
        it("Liquidation works also for solely borrow entry", async () => {
            ...
            const paybackUSD = await contracts.oracleHub.getTokensInUSD(
                WETH.address,
                paybackAmount
            );

            const valueBefore = await contracts.lending.positionPureCollateralAmount(
                1,
                USDC.address
            );

            const collatUSD = await contracts.security.overallUSDCollateralsBoth(
                1
            );
            ...
            const liquReward = Bi(paybackUSD)
                * Bi(feeLiquidation)
                / Bi(toWei("1"));
            ...
            const valueAfter = await contracts.lending.positionPureCollateralAmount(
                1,
                USDC.address
            );
            ...
            const borrowUSDAfter = await contracts.security.overallUSDBorrow.call(
                1
            );
        ...
        it("Liquidation works also for solely borrow entry plus normal collat deposit", async () => {
            ...
            */
            ...
            const borrowUSD = await contracts.security.overallUSDBorrow.call(
                1
            );
            ...
            const valuePureCollatToken2 = await contracts.lending.positionPureCollateralAmount(
                1,
                USDC.address
            );

            const valuePureCollatToken1 = await contracts.lending.positionPureCollateralAmount(
                1,
                WETH.address
            );
            ...
            const collatUSDWish = await contracts.security.getFullCollateralUSD(
                1,
                USDC.address
            );
            ...
            const collatUSDWishAfter = await contracts.security.getFullCollateralUSD(
                1,
                USDC.address
            );
            ...
            const valuePureAfterToken1 = await contracts.lending.positionPureCollateralAmount(
                1,
                WETH.address
            );
    ...
    describe("Liquidation tests", () => {
        ...
        it("Liquidator gets right token amount from liquidation", async () => {
            ...
            const collatAliceUSD = await contracts.security.getFullCollateralUSD(
                1,
                WETH.address
            );

            const collatAliceToken = await contracts.lending.positionPureCollateralAmount(
                1,
                WETH.address
            );
            ...
            const feeLiquidation = await contracts.security.baseRewardLiquidation();
            const maxFeeUSD = await contracts.security.maxFeeUSD();
            ...
            const collatAliceTokenAfter = await contracts.lending.positionPureCollateralAmount(
                1,
                WETH.address
            );

            const diffAliceToken1 = Bi(2)
                * (Bi(collatAliceToken)
        ...
        it("Liquidator get right amount if shares when pool has not enough token", async () => {
            ...
            const totalPoolToken1USD = await contracts.oracleHub.getTokensInUSD(
                WETH.address,
                totalPoolWETH
            );

            const feeUSD = Bi(payBackUSD)
                * Bi(fee)
                / Bi(toWei("1"));
            ...
            assert.equal(
                inverseFraction.toString(),
                liquidationPercentage.toString()
            );

            const rewardAmountChadSharesUSD = await contracts.security.getUSDCollateral(
                3,
                WETH.address
            );
            ...
            const sumBareUSD = Bi(rewardAmountChadSharesUSD)
                + Bi(totalPoolToken1USD)
                - Bi(feeUSD);

            console.log(sumBareUSD.toString(), 'sumBareUSD')
            console.log(payBackUSD.toString(), 'payBackUSD')

            const diff = Bi(sumBareUSD) - Bi(payBackUSD);
        ...
        it("Max fee check works correctly", async () => {
            ...
            const totalCollat = await contracts.security.overallUSDCollateralsWeighted.call(
                1
            );
            ...
            const paybackUSDSmall = await contracts.oracleHub.getTokensInUSD(
                USDC.address,
                paybackTokenSmall
            );
            ...
            const feeSmallUSD = Bi(paybackUSDSmall)
                * Bi(feeLiquidation)
                / Bi(toWei("1"));
            ...
            const diffBalUSD = await contracts.oracleHub.getTokensInUSD(
                WETH.address,
                diffBal
            );
            ...
            const diff = Bi(maxFeeUSD)
                - Bi(feeAmountUSD);

            console.log(maxFeeUSD.toString(), 'maxFeeUSD');
            console.log(paybackUSD.toString(), 'paybackUSD');
    ...
    describe("Liquidation threshold and allowBorrow tests", () => {
        ...
        it("User can borrow only up to liquidation threshold (normal pool)", async () => {
            ...
            const borrowPercentageCap = await contracts.security.borrowPercentageCap();

            const collatWheigtedUSD = await contracts.security.overallUSDCollateralsWeighted(
                1
            );

            const allowedBorrowAmountUSD = Bi(collatWheigtedUSD)
                * Bi(borrowPercentageCap)
                / Bi(toWei("1"));

            const allowedBorrowAmount = await contracts.oracleHub.getTokensFromUSD(
                WETH.address,
                allowedBorrowAmountUSD
            );
        ...
        it.skip("User can borrow only up to liquidation threshold (isolation pool)", async () => {
            ...
            await setLastUpdateGlobal(
                [chainlinkWETH, chainlinkUSDC, chainlinkUSDT]
            );
            ...
            await contracts.lending.approve(
                1,
                isolationPool.address,
                USDC.address,
                HUGE_AMOUNT,
            ...
            const borrowPercentageCap = await isolationPool.borrowPercentageCap();

            const collatWheigtedUSD = await isolationPool.getTotalWeightedCollateralUSD(
                1
            );
            ...
            const tinyBiggerAmount = Bi(allowedBorrowAmountUSD)
                + Bi(toWei("0.000000000000000001"));
