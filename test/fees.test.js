...
contract("WiseLending feeDestination", async accounts  => {
    ...
    describe("Basic FeeManager tests", () => {
        ...
        it("Can't withdraw with open bad debt", async () => {
            ...
            const badDebtUsd = await contracts.feeManager.totalBadDebtUSD();
        ...
        it("User can payback bad debt and gets right fee token amount", async () => {
            ...
            const ALLOWED_DIFFERENCE_USD = toWei("0.001");
            ...
            const badDebtContract = await contracts.feeManager.totalBadDebtUSD();
            ...
            const badDebtContractAfter = await contracts.feeManager.totalBadDebtUSD();
            ...
            const diffUSDUser = Bi(paybackUSD)
                - Bi(diffBaddebtUser);

            const boolDiffUSD = parseInt(abs(diffUSDUser)) < parseInt(ALLOWED_DIFFERENCE_USD);

            assert.equal(
                boolDiffUSD.toString(),
                "true"
            );
        })
        ...
        it("Bad debt updates correctly when token value changes", async () => {
            ...
            await contracts.feeManager.paybackBadDebtForFree(
                1,
                USDC.address,
                borrowSharesUser,
                {
