...
contract("WiseLending ScalingAlgorithm test", (accounts) => {
    ...
    describe("getter and Setter tests", () => {
        ...
        it("timeStampScaling works as intended", async () => {
            ...
            const oldTimestamp1 = await contracts.lending.getTimestampScaling(
                WETH.address
            );
            ...
            const oldTimestamp2 = await contracts.lending.getTimestampScaling(
                WETH.address
            );

            assert.equal(
                oldTimestamp1.toString(),
                oldTimestamp2.toString()
            );
            ...
            const oldTimestamp3 = await contracts.lending.getTimestampScaling(
                WETH.address
            );

            assert.isAbove(
                parseInt(oldTimestamp3),
                parseInt(oldTimestamp2)
            );
        });
    });
    ...
    describe("LASA parameter set function tests", () => {
        ...
        it("Parameters get updated correctly after function call", async () => {
            ...
            const newUpperBound = toWei("300");
            const newLowerBound = toWei("100");
