...
contract("Isolation Mode Advanced Tests", async accounts  => {
    ...
    describe("Advanced Tests with two Borrow Token (50/50)", () => {
        ...
        it("borrowExactDebtRatio gets the same token amounts as borrowExactUSD", async() => {
            ...
            const borrowAmountDebtRatioDAIUSD = await contracts.oracleHub.getTokensInUSD(
                DAI.address,
                balAfterDAI
            );

            const borrowAmountDebtRatioUSDCUSD = await contracts.oracleHub.getTokensInUSD(
                USDC.address,
                balAfterUSDC
            );
        ...
        it("paybackExactDebtratio payback same amount as paybackExactUSD", async() => {
            ...
            const usdEquivDAI = await contracts.oracleHub.getTokensInUSD(
                DAI.address,
                callBackValue[0]
            );

            const usdEquivUSDC = await contracts.oracleHub.getTokensInUSD(
                USDC.address,
                callBackValue[1]
            );
        ...
        it("Liquidation works correctly for two token", async() => {
            ...
            const diffDAIUSD = await contracts.oracleHub.getTokensInUSD(
                DAI.address,
                diffDAI
            );

            const diffUSDCUSD = await contracts.oracleHub.getTokensInUSD(
                USDC.address,
                diffUSDC
            );
            ...
            const aliceCollatUSD = await contracts.oracleHub.getTokensInUSD(
                LPToken.address,
                LPtokenAliceAfter
            );
            ...
            const USDWithoutFee = Bi(aliceCollatUSD)
                * Bi(toWei("1"))
                / Bi(liquFeeInverse);
