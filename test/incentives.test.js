...
contract("Incentive tests", async accounts => {
    ...
    describe("Incentive tests", () => {
        ...
        it("Incentive Address can only changed by owner", async () => {
            ...
            const startValueA = toWei("196000");
            const startValueB = toWei("220000");
        ...
        it("Incentive can only be increased", async () => {
            ...
            const incentiveUSDA = await contracts.feeManager.incentiveUSD(
                userA
            );
            ...
            const incentiveUSDAafter = await contracts.feeManager.incentiveUSD(
                userA
            );

            const diff = Bi(incentiveUSDA)
                - Bi(incentiveUSDAafter);
            ...
            assert.isAbove(
                parseInt(incentiveUSDAafter),
                parseInt(incentiveUSDA)
            );

            const incentiveUSDB = await contracts.feeManager.incentiveUSD(
                userB
            );
            ...
            const incentiveUSDBafter = await contracts.feeManager.incentiveUSD(
                userB
            );

            const diff2 = Bi(incentiveUSDB)
                - Bi(incentiveUSDBafter);
            ...
            assert.isAbove(
                parseInt(incentiveUSDBafter),
                parseInt(incentiveUSDB)
            );
        });
        ...
        it("Incentive get calculated and accounted correctly", async () => {
            ...
            const usdEquivIncentivesUSDC = await contracts.oracleHub.getTokensInUSD(
                USDC.address,
                portionUserAusdc
            );

            const usdEquivIncentivesWETH = await contracts.oracleHub.getTokensInUSD(
                WETH.address,
                portionUserAweth
            );
        ...
        it("Only max USD equiv gets distributed for incentives", async () => {
            ...
            const usdEquiv = await contracts.oracleHub.getTokensInUSD(
                WETH.address,
                gatheredToken
            );

            assert.equal(
                initalIncentiveUSD.toString(),
                usdEquiv.toString()
            );
        });
