...
const CHAINLINK_DAI = "0xAed0c38402a5d19df6E4c03F4E2DceD6e29c1ee9";
...
contract("Aave second layer", async accounts => {
    ...
    describe("Extra security and update tests", () => {
        ...
        it("Can't use aaveHub functions when registered for isolation pools", async () => {
            ...
            await expectRevert(
                aavePools.paybackExactAmountETH(
                    aliceNFT,
                    {
                        from: alice,
                        value: testAmount
        ...
        it("Collateral and borrow amount get correctly updated when aave pools used", async () => {
            ...
            const collatAlice = await contracts.security.overallUSDCollateralsWeighted(
                aliceNFT
            );
            ...
            const collatAlice2 = await contracts.security.overallUSDCollateralsWeighted(
                aliceNFT
            );
            ...
            const borrowUSD = await contracts.security.overallUSDBorrow(
                aliceNFT
            );
            ...
            const borrowUSDEND = await contracts.security.overallUSDBorrow(
                aliceNFT
            );

            const collatAliceEND = await contracts.security.overallUSDCollateralsWeighted(
                aliceNFT
            );
            ...
            const reducedBorrowUSD = Bi(borrowUSDEND)
                - Bi(borrowETHUSD)

            assert.isAbove(
                parseInt(reducedBorrowUSD),
                parseInt(borrowUSD)
            );
        });
    });
    ...
    describe("Aave pools liquidation tests", () => {
        ...
        it("Liquidation works for aToken (as payback)", async () => {
            ...
            const depositDummyOwner = toWei("100");
            const depositDummy = toWei("100000");
            const borrowUSDC = pow6(150);
