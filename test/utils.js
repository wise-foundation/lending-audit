...
setupHeartbeatForTests = async (inputParam = {}) => {
    ...
    const phaseId = 1;
    const aggregatorRoundMax = 3;
    const timeDistances = [84000, 89000, 93000];
    ...
    for (i = 1; i <= aggregatorRoundMax; i++){
        ...
        currentRoundId = await oracleHub.getRoundIdByByteShift(
            phaseId,
            i
        );
        ...
        for (index = 0; index < chainlinkInterfaces.length; index++) {
            ...
            distance = currentTime.add(Bn(timeDistances[i-1]));
